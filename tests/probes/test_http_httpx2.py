...
async def test_success_with_client() -> None:
    ...
    probe = HttpHttpx2Probe('http://svc/health', client=make_client(200, seen))

    result = await probe.run_check()

    assert result.ok is True
    assert result.name == 'HttpHttpx2Probe'
...
async def test_unexpected_status_is_failure() -> None:
    probe = HttpHttpx2Probe('http://svc/health', client=make_client(503, []))
...
async def test_client_factory() -> None:
    ...
    result = await HttpHttpx2Probe('http://svc/health', client=factory).run_check()
...
async def test_transport_error_is_reported() -> None:
    ...
    result = await HttpHttpx2Probe('http://svc/health', client=client).run_check()
